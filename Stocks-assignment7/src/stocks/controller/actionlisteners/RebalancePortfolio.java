 ...
 * passes it the model.
 */
public class RebalancePortfolio implements ActionListener {
   ...
   */
  public RebalancePortfolio(JDialog rebalancePortfolioDialog, PortfolioUIView view,
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    Set<String> tickers = model.getComposition(name, date).keySet();
    if (tickers.size() == 0) {
      view.createErrorDialog(String.format("No stocks found in portfolio on date %s.", date),
          ...
          if (symbolWeightCheckBox.isSelected()) {
            ...
            confirmRebalance.addActionListener(e1 -> {
                  try {
                    ...
                    view.createErrorDialog(ex.getMessage(), rebalancePortfolioDialog);
                  }
                }
            );
